...
## Data attribution — required reading before reusing these figures

**`before-after.png` contains third-party tomographic data and is not covered by
this repository's MIT licence.** The two raster panels are renders of the
`PHerc0009B` volume from the Vesuvius Challenge Open Data bucket. The full
attribution — who holds the rights, the source links, the terms, both dataset
citations and an itemised description of the transformations applied — is in
[`../../DATA_ATTRIBUTION.md`](../../DATA_ATTRIBUTION.md), linked from the repository
`README.md` and from [`../../NOTICE.md`](../../NOTICE.md) §3.

In brief, and without replacing that file:

* The published Vesuvius Challenge datasets are **CC BY-NC 4.0** unless otherwise
  noted for specific assets (`scrollprize.org/docs/02_data.md`; `DOCS/RESEARCH.md`
  §1). CC BY-NC 4.0 requires attribution, a link to the licence, an indication of
  modifications, and non-commercial use.
* `PHerc0009B` was scanned in 2025 at ESRF BM18 and belongs to the
  **Vesuvius Challenge – CT Scans of Herculaneum Papyri** dataset; `PHerc0172` is
  legacy-shaped and belongs to **EduceLab-Scrolls**, copyrighted by EduceLab / The
  University of Kentucky. The two datasets have different required citations, both
  given in `DATA_ATTRIBUTION.md` §3. That assignment is derived from each volume's
  own metadata document — the derivation and its one weak point are recorded there.
* `terminal-before-after.png` embeds **no** raster data from the volumes; it is this
  project's own CI output.

`../../LICENSE` (MIT) does not apply to the panels above, and nothing here
relicenses any third-party material.

## Provenance of every value in each figure, and how to regenerate them

Already stated per figure above. In addition, for completeness:

* The bucket metadata documents committed under `research/raw_metadata/` are
  verbatim copies of the published documents, with URLs and SHA-256 in
  `research/raw_metadata/PROVENANCE.md`.
* The committed figures are generated by `ci/build_evidence_figure.py` and
  `ci/build_terminal_evidence.py`; each has a self-test that re-derives its inputs
  from the run's artefacts, so the claims about them are checkable rather than
  asserted.
* The committed `before-after.png` is **not byte-identical** to a fresh generation:
  the residue is font rasterisation, under 1% of pixels, and **none** of it falls
  inside the raster panels. `ci/selftest_evidence_figure.py` asserts both halves of
  that sentence.
